...
<section
    ...
    @if(!empty($section_id)) id="{{ $section_id }}" @endif
    @class([ 
        'b-hero bg-background relative text-white pt-[90px] pb-[310px]',
        $sectionClass => filled($sectionClass),
        $section_class => filled($section_class),
        $background => filled($background) && $background !== 'none',
    ])>
...
<div class="absolute z-4 right_shape"><img class="" src="{{ get_template_directory_uri() }}/resources/images/shape.svg"></div>
    ...
    <div class="c-main mx-auto px-4 flex flex-col gap-12">
        <div class="w-full text-left">
            ...
            <h1 class="hero-main-title flex flex-wrap items-baseline gap-y-2 w-full md:w-2/3">
                <span class="title-part ">{{ $g_hero['title'] ?? '' }}</span>
                
                @if($wordCount > 0)
                    <span class="text-carousel-container">
                        <span class="text-carousel-slider " style="--steps: {{ $wordCount }};">
                            @foreach($carouselWords as $word)
                                <span class="text-carousel-item uppercase">{{ $word }}</span>
                            @endforeach
...
<video
    ...
    loop
    playsinline
    class="w-full h-[500px] object-cover -mt-[250px] radius relative z-3">
    <source src="{{ trim($g_hero['video'], '"') }}" type="video/mp4">
</video>
</div>

<section class="bg-bg relative z-10 py-20 md:py-40">
    <div class="c-main grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-12 items-center">
        
        <div class="md:col-span-9 flex flex-col text-left">
            @if(!empty($g_hero['title_sub']))
                <span class="block text-sm md:text-base  text-secondary-500 m-title">
                    {{ $g_hero['title_sub'] }}
                </span>
            ...
            <div data-gsap-element="text" class="text-h3 md:text-5xl text-carbon">
                {!! $g_hero['text'] !!}
            </div>
        </div>

        <div class="md:col-span-3 flex justify-center md:justify-end w-full ">
            <img 
                src="{{ $g_hero['image']['url'] }}" 
                alt="{{ $g_hero['image']['alt'] }}" 
                class="w-full max-w-[266px] h-auto object-contain shrink-0" 
