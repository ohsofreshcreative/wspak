<section
	...
	<div class="__wrapper c-main">
		...
		<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
			...
			<div class="portfolio-card p-4 bg-white radius flex flex-row justify-between">
				<div class="flex items-start gap-4">
					@if(!empty($item['cover']))
					<img src="{{ $item['cover'] }}" alt="{{ $item['title'] ?? '' }}" class="w-19 h-19 rounded-full object-cover flex-shrink-0">
					@endif
					<div class="flex flex-col">
						...
						<div class="flex flex-col gap-2">
							<div class="grid grid-cols-2 ">
								...
								<p class="!text-base !font-semibold pr-4">
				...
				<div class="flex-shrink-0 ml-4">
					@if(!empty($item['badge']))
					<span class="bg-primary-25 text-primary-900 text-xs !font-semibold px-2 py-1 rounded-[48px] inline-block whitespace-nowrap">{{ trim($item['badge']) }}</span>
					@endif
				</div>
			</div>
			@endforeach
